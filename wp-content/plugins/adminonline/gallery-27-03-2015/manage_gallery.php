...
##########################################	
		...
		else 
		{
			...
			if($selectcategory_id){
				...
				$_REQUEST['subcategory_id'] = $subcat_id;
				$sql="select * from tbl_gallery_main gm inner join tbl_gallerysubcategory c on gm.subcategory_id=c.subcategory_id where gm.category_id=".$fistcatId." AND gm.subcategory_id=".$subcat_id." order by gm.$sort $mode ";	
				$sql = $wpdb->prepare( $sql,"" ) ;
				$count_sql = "select count(*) from tbl_gallery_main gm inner join  tbl_gallerysubcategory c on gm.subcategory_id=c.subcategory_id where 1=1 AND  gm.category_id=".$fistcatId." AND gm.subcategory_id=".$subcat_id;
				$count_sql = $wpdb->prepare( $count_sql,"" ) ;
			}
			else{
				$count_sql = 0;
...
<?php */?>		 
			...
			{?>
				<th>Category</th>
			<?php }?>
          <?php /*?> <th width="10%" class="<?php echo $field_sort.' '.$class_sort; ?>">Main Title</th><?php */?>
      ...
      <tbody>
        ...
        <tr id="<?php echo $rec_list[$i]->$primary_key; ?>">
          ...
          <td width="20%"><?php
			 ...
			 ?></td>
			 <?php }?>
       <?php /*?>    <td width="17%">Patient <?php echo $cnt;?></td><?php */?>
		 <td width="11%">
		 	<?php 
