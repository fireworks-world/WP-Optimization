...
<?php
	...
	if($searchText!='') {
	     ...
	     $where = " and (author LIKE %s OR testimony LIKE %s) ";
    ...
    <table width="100%" cellspacing="0" class="widefat" <?php if($searchText=='' && $tot_cnt>1 && $sort=='ordering') { ?> id="table-1" <?php } ?>>
      ...
      <thead>
        ...
        <tr class="nodrag nodrop">
          ...
          <th width="364" class="<?php echo $field_sort.' '.$class_sort; ?>">

		  	<a href="<?php echo get_option('siteurl').'/wp-admin/admin.php?page='.$page_name.'&sort=author'; ?>&mode=<?php echo $mode; ?>&searchText=<?php echo $searchText; ?>&ipp=<?php echo $ipp; ?>&pval=<?php echo $pval; ?>" title="Sort by Author">
          ...
          <th width="331" class="<?php echo $field_sort.' '.$class_sort; ?>">

		  	<a href="<?php echo get_option('siteurl').'/wp-admin/admin.php?page='.$page_name.'&sort=testimony'; ?>&mode=<?php echo $mode; ?>&searchText=<?php echo $searchText; ?>&ipp=<?php echo $ipp; ?>&pval=<?php echo $pval; ?>" title="Sort by Testimonial">
          ...
          <th width="114">Edit</th>

          <th width="172" align="right" style="text-align:right;">
      ...
      <tbody>
        ...
        <tr id="<?php echo $rec_list[$i]->$primary_key; ?>">
          ...
          <td width="73" align="left" style="padding-left:25px;"><?php echo $cnt;?></td>

          <td width="364"><?php

			  	$author = strip_tags($rec_list[$i]->author);

				if(strlen($author)>35)

					echo substr($author,0,35).'...';
          ...
          <td width="331"><?php 
			  	...
			  	$testimony = strip_tags($rec_list[$i]->testimony);

				if(strlen($testimony)>35)

					echo substr($testimony,0,35).'...';
		 ...
		 ?>          </td>
		  ...
		  <td width="265"><?php echo ($rec_list[$i]->status=='N')?'Inactive':'Active'; ?></td>

          <td width="114"><a href="<?php echo get_option('siteurl').'/wp-admin/admin.php?page='.$page_name.'&'.$primary_key.'='.$rec_list[$i]->$primary_key;?>&option=add"  title="Edit this record">Edit</a></td>

          <td width="172" align="right" style="padding-right:8px;"><input type="checkbox" name="del" value="<?php echo $rec_list[$i]->$primary_key;?>" style="cursor:default" /></td>
