...
error_reporting(E_ALL ^ E_NOTICE);
	...
	*/
	$query = "SELECT COUNT(*) as num FROM  tbl_faq where status='Y' ORDER BY ordering ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages['num'];
	//print_r($total_pages);
	
	/* Setup vars for query. */
	$targetpage = get_option('siteurl').'/6-month-braces/faq';
	...
	$sql_faq = "select * from   tbl_faq where status='Y' ORDER BY ordering asc LIMIT $start, $limit";
    $result_faq = mysql_query($sql_faq);
    $count_num_faq = mysql_num_rows($result_faq); 
