<?php
$tmp_url_prod1 = explode('/?pages=', $url);
$pages = $tmp_url_prod1[1];
$category_id=18;
error_reporting(E_ALL ^ E_NOTICE);
	...
	*/
	$query = "SELECT DISTINCT  COUNT(Distinct subcategory_name ) as num FROM tbl_gallerysubcategory c inner join tbl_gallery_main gm on c.subcategory_id=gm.subcategory_id  where c.status='Y' and gm.status='Y' AND c.category_id = '$category_id' ORDER BY c.ordering
";
	...
	$targetpages = get_option('siteurl').'/smile-gallery/'.$category_seotitle.'/';
	...
	//$sql = "SELECT * FROM $tbl_name1 where status='Y'  order by ordering asc LIMIT $start, $limit";
 	//$result = mysql_query($sql);
	   ...
	   $sql_cat = "SELECT DISTINCT subcategory_name,c.*  FROM tbl_gallerysubcategory c inner join tbl_gallery_main gm on c.subcategory_id=gm.subcategory_id  where c.status='Y' and gm.status='Y' AND c.category_id = '$category_id' ORDER BY c.ordering asc LIMIT $start, $limit";
	...
	*/
	...
	{	
		...
		//next button
		if ($pages < $counter - 1) 
			$pagination.= "<a href=\"$targetpages?pages=$next\" class=\"pag-next\"></a>";
			
			//$pagination.= "<a href=\"$targetpages/$next/\">Next &raquo;</a>";
		else
			$pagination.= "<span class=\"disabled disable-next\">Next &raquo;</span>";
		$pagination.= "</div>\n";		
	}
